...
/**
 * Created by bimalchawla on 4/1/17.
 */

public class SecurityQuestionActivity extends AppCompatActivity implements View.OnClickListener, ApiResponseListener {
    private ImageView back;
    private View submitView;
    private ProgressDialog progressDialog;
    private EditText question, answer;
    private String ques, ans, token = "", userId = "";
    ...
    private void initIds() {
        submitView = findViewById(R.id.btn_submit_securtiy_question);

        question = (EditText) findViewById(R.id.security_question);
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.btn_submit_securtiy_question:
    ...
    private boolean validation() {
        ...
        if (ques.length() < 0 || ques.length() == 0) {
            Utility.showToast(this, "Please enter your security question");
            return false;
        } else if (ans == null || ans.length() == 0) {
            Utility.showToast(this, "Please enter your security answer");
            return false;
    ...
    private void UploadDetail() {
        ...
        header.put(ApiConstants.AUTH_TOKEN_KEY, token);
            ...
            ApiCall.getInstance().makePostRequest(ApiConstants.API_ADDRESS.SECURITY_QUESTION.path, params, header, this, 101, userId);
...
//            Log.i("response", response);
    ...
    public void onError(String error, int errorCode, int requestCode) {
        ...
        if (error != null) {
            try {
                if (errorCode == 402) {
                    AppSharedPrefrence.clearAllPrefs(SecurityQuestionActivity.this);
                    Intent login = new Intent(SecurityQuestionActivity.this, PaymentActivity.class);
